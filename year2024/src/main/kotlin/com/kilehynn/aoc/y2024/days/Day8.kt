...
class Day8 :  BaseDay() {
    override var expectedPart1:Long? = 280L
    override var expectedPart2 :Long? =958L
    ...
    override fun parse(input :String){
        nbCols = input.indexOf(System.lineSeparator())
        this.input= input.replace(System.lineSeparator(), "")
        nbRows = this.input.length / nbCols
        val antennaSymbol = regexAntenna.findAll(this.input).map { it.value[0] }.distinct().toList()
        for (i in this.input.indices) {
            if (this.input[i] in antennaSymbol) {
                val antennasArray = antennas.getOrDefault(this.input[i], mutableListOf())
                antennasArray.add(Antenna(i % nbCols, i / nbCols, this.input[i]))
                antennas[this.input[i]] = antennasArray
            }
        }
